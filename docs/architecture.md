...
- `sayit-service` owns synthesis, playback, models, voices, settings, jobs, and history. It binds only to loopback and publishes a versioned protocol.
- `sayit` is a service client. Direct, one-shot model execution remains available only as a development/diagnostic command.
